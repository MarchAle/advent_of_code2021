...
bool	already_in_path(std::string &name, std::vector<std::string> &current_path)
{
	std::vector<std::string>::iterator end = current_path.end();
	for (std::vector<std::string>::iterator it = current_path.begin(); it != end; it++)
	{
...
void	set_all_path_as_visited(cave_system &cave_system, std::vector<std::string> &current_path)
{
	std::vector<std::string>::iterator path_end = current_path.end();
	for (std::vector<std::string>::iterator path_cave = current_path.begin(); path_cave != path_end; path_cave++)
	{
		cave_system::iterator end = cave_system.end();
		for (cave_system::iterator it = cave_system.begin(); it != end; it++)
		{
			if ((*it).first.name.compare(*path_cave) == 0 && is_small_cave(*path_cave))
				(*it).first.visited = 1;
		}
	}
}

void	set_as_visited(cave_system &cave_system, std::string &name, std::vector<std::string> &current_path)
{
	if (is_small_cave(name) && name.compare("end") != 0)
	{
		if (!already_in_path(name, current_path) && double_visited.compare("") == 0)
			return;
		else if (already_in_path(name, current_path) && double_visited.compare("") == 0)
		{
			double_visited = name;
			set_all_path_as_visited(cave_system, current_path);
		}
		cave_system::iterator end = cave_system.end();
		for (cave_system::iterator it = cave_system.begin(); it != end; it++)
		{
...
void	set_all_path_as_not_visited(cave_system &cave_system, std::vector<std::string> &current_path)
{
	std::vector<std::string>::iterator path_end = current_path.end();
	for (std::vector<std::string>::iterator path_cave = current_path.begin(); path_cave != path_end; path_cave++)
	{
		set_as_not_visited(cave_system, *path_cave);
	}
}
...
void	explore_system(cave_system::iterator &node, cave_system &cave_system, paths &paths, std::vector<std::string> &current_path)
{
	...
	for (std::vector<std::string>::iterator it = (*node).second.begin(); it != (*node).second.end(); it++)
	{
		...
		{
			set_as_visited(cave_system, *it, current_path);
	...
	if (current_path.back().compare(double_visited) == 0)
	{
		double_visited = "";
		set_all_path_as_not_visited(cave_system, current_path);
	}
	current_path.pop_back();
	return;
}
