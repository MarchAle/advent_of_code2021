...
{
	t_area			area;
	std::vector<int> max_height_recorded;
	std::string		line;
	std::string		packet;
	...
	for (int i = 0; i < 150; i++)
	{
		for (int j = 0; j < 150; j++)
		{
			...
			if (result.first == 1)
				max_height_recorded.push_back(result.second);
		}
	}
	std::cout << BGREEN << *(std::max_element(max_height_recorded.begin(), max_height_recorded.end())) << END << std::endl;
}